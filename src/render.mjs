...
import { briefBody } from './brief-page.mjs';
...
export const renderHome = () => {
  ...
  const body = `
    ...
    <section class="section shell brief-entry" aria-labelledby="brief-entry-title">
      ${indexLine('06', 'Оценка проекта')}
      <div class="section-heading"><h2 id="brief-entry-title">Получить оценку<br><em>проекта.</em></h2><p>Ответьте на несколько вопросов – я смогу точнее оценить объём, сроки и стоимость.</p></div>
      <a class="button button--accent" href="/brief/">Заполнить бриф <span aria-hidden="true">↗</span></a>
    </section>
    ${contactCta()}`;
...
export const renderBrief = () => shell({
  title: 'Бриф на создание сайта — Кирилл Снежин',
  description: 'Заполните короткий бриф на создание сайта. Я изучу задачу, предложу подходящий формат, примерные сроки и стоимость проекта.',
  path: '/brief/', current: '', body: briefBody(site.telegram), bodyClass: 'brief-page-body', schema: breadcrumbSchema([{ label: 'Главная', href: '/' }, { label: 'Бриф', href: '/brief/' }])
});

export const renderPrivacy = () => {
  const body = `
    <article class="legal-page shell"><nav class="breadcrumbs" aria-label="Хлебные крошки"><a href="/">Главная</a><span>/</span><span>Обработка данных</span></nav>${indexLine('Legal', 'Информация об обработке данных')}<h1>Обработка персональных данных</h1><p class="lead-copy">Актуально на 9 сентября 2026 года. Сайт не использует аналитику и рекламные cookie. При добровольной отправке брифа данные передаются в сервис Formspree для доставки обращения оператору.</p><section><h2>Оператор и связь</h2><p>Оператор — Кирилл Снежин. По вопросам, связанным с обращениями и персональными данными, можно написать на <a href="${emailHref()}">${site.email}</a>.</p></section><section><h2>Какие данные могут обрабатываться</h2><p>В брифе или добровольном сообщении могут быть указаны имя, контакты и описание проекта. Не отправляйте пароли, платёжные данные, документы, медицинскую информацию и иные чувствительные сведения.</p></section><section><h2>Цель и основание</h2><p>Данные используются только для ответа, обсуждения возможного проекта и дальнейшей переписки по нему. Для рассылок, передачи данных третьим лицам или публикации обращений сайт их не использует.</p></section><section><h2>Сторонние сервисы и технические данные</h2><p>Бриф передаётся в Formspree как техническому обработчику формы; его правила обработки данных действуют отдельно. Черновик брифа хранится только в браузере посетителя до 7 дней и удаляется после успешной отправки. Переход по ссылке на почту, Telegram или VK открывает выбранный вами внешний сервис. Статический хостинг может вести технические журналы запросов, включая IP-адрес и сведения браузера.</p></section><section><h2>Права и срок хранения</h2><p>Вы можете запросить сведения об обращении, уточнение или удаление данных, написав на <a href="${emailHref()}">${site.email}</a>. Обращения хранятся не дольше, чем это необходимо для ответа и дальнейшей рабочей переписки, если более длительный срок не требуется законом.</p></section><section><h2>Что требуется подтвердить владельцу</h2><p>Для окончательной юридической проверки нужно подтвердить статус оператора, почтовый адрес для обращений, место хранения переписки и технических журналов, а также применимость требований конкретной юрисдикции. Эта страница описывает фактическую схему сайта, но не заменяет консультацию юриста.</p></section></article>`;
  return shell({
...
export const render404 = () => compactDashes(`<!doctype html><html lang="ru"><head>${pageHead({ title: 'Страница не найдена — snezhin.design', description: 'Запрошенная страница не найдена.', path: '/404.html', robots: 'noindex, follow' })}</head><body class="not-found-page">${header('')}<main id="main"><section class="not-found shell"><div class="not-found__copy">${indexLine('404', 'Страница не найдена')}<h1>Кажется, эта<br><em>страница ушла.</em></h1><p>Возможно, адрес изменился или в ссылке опечатка. Можно вернуться на главную, посмотреть работы или начать с короткого брифа.</p><div class="not-found__actions"><a class="button button--accent" href="/">На главную <span aria-hidden="true">↗</span></a><a class="text-link" href="/portfolio.html">Посмотреть работы <span aria-hidden="true">→</span></a><a class="text-link" href="/brief/">Заполнить бриф <span aria-hidden="true">→</span></a></div></div><p class="not-found__number" aria-hidden="true">404</p></section></main>${footer()}</body></html>`);
